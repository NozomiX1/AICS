...
class ConvolutionalLayer(object):
            ...
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        input_region = self.input_pad[idxn, :, idxh * self.stride:idxh * self.stride + self.kernel_size,
                                                               idxw * self.stride:idxw * self.stride + self.kernel_size]

                        output_value = np.sum(input_region * self.weight[:, :, :, idxc]) + self.bias[idxc]
                        # print(output_value.shape)
                        self.output[idxn, idxc, idxh, idxw] = output_value
    ...
    def forward_speedup(self, input):
        ...
        output = np.matmul(self.weights_col, self.input_col.reshape(self.input_col.shape[0], -1,
                                                                    self.input_col.shape[3])) + self.bias.reshape(-1, 1)
...
class MaxPoolingLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        self.input_col = img2col(self.input, self.height_out, self.width_out, self.kernel_size, self.stride).reshape(
            self.input.shape[0], self.input.shape[1], -1, self.height_out, self.width_out)
                ...
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        max_index = np.argwhere(
                            self.max_index[idxn, idxc, idxh * self.stride: idxh * self.stride + self.kernel_size,
