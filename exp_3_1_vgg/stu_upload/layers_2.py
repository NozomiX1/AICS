            ...
            for idxc in range(self.input.shape[1]):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        input_region = self.input[idxn, idxc, idxh * self.stride:idxh * self.stride + self.kernel_size,
                                                              idxw * self.stride:idxw * self.stride + self.kernel_size]
                        output_value = np.max(input_region)
                        index = np.argmax(input_region)
                        self.output[idxn, idxc, idxh, idxw] = output_value
                        self.max_index[idxn, idxc, idxh * self.stride + index // self.kernel_size,
                                                   idxw * self.stride + index % self.kernel_size] = 1
        return self.output
